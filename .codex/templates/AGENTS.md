...
9. **Design decisions precede implementation.** For any task involving visual appearance, interaction patterns, or UX choices — route to the `designer` first to produce a spec, then hand that spec to the `builder` (frontend) to implement. The orchestrator must never specify CSS values, colors, or visual details itself.
...
11. **When invoking `builder`, specify the domain in your request** (e.g. "frontend task — add dark mode toggle" or "database task — add index on orders table"). The builder reads the corresponding skill before starting work.

---

## Prompt Packs

Use these prompt files or direct instructions to trigger common multi-step workflows:
...
You can declare optional hooks in `.codex/hooks.json` to enforce conventions if your Codex setup loads project-local hooks:
...
| `validate-completion.sh` | When Codex finishes a turn | Warns if implementation files changed but `docs/` was not updated, or if `TODO.md` was not updated |
| `log-agent.sh` | When a subagent starts | Appends a timestamped line to `.codex/agent-log.txt` for an audit trail |
...
Rules in `.codex/rules/` inject context automatically based on the file being edited:
...
.codex/
  ...
  prompts/             # Prompt files (orchestrate, status, start, sync-template)
  ...
  .codex-plugin/       # Plugin manifest — makes skills available when symlinked into ~/.codex/plugins/
