...
3. **Follow conventions in AGENTS.md**: Formatting, import style, naming conventions.
...
If the project has a linter configured (ESLint, Biome, etc.) or a formatter (Prettier), run it after UI edits.

Recommended flow:
1. Auto-format the edited file if a formatter is configured (`prettier --write`, `biome format --write`)
2. Run the linter on the edited file or package scope
3. Treat lint errors as blocking feedback and fix them before continuing
4. If no formatter or linter config exists yet, note that explicitly instead of inventing tooling
