...
name: backend
description: >
  Use when creating or modifying API endpoints, implementing business logic,
  handling authentication or authorization, building background jobs, integrating
  third-party services, or optimizing server-side performance and caching.
---
...
3. **Execute migrations using the project's tool**: When a schema spec is provided (forward DDL + rollback DDL + deployment risk notes), wrap it in the project's migration tool. Detect the tool from `AGENTS.md` and project dependencies:
...
5. **Validate all inputs**: Every endpoint must validate and sanitize input with Zod or equivalent. No raw user data reaches the database.
6. **Enforce authentication**: All endpoints require authentication unless a FR-XXX requirement in PRD.md explicitly marks them public.
7. **Implement in layers**: Handler → Service → Repository. Business logic lives in the service layer, not in the handler.
8. **Update API.md immediately**: Before marking the task complete, update `docs/technical/API.md` with the new/modified endpoint.
9. **Write tests**: Unit tests for business logic (pure functions, domain services), integration tests for endpoints. Run them and confirm they pass.
